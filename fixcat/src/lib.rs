...
pub fn read_standard_header(br: &mut dyn BufRead) -> std::io::Result<StandardHeader> {
    let buf = br.fill_buf()?;
    if buf.len() == 0 {
        ()
    };
    ...
    assert!(body_length.0 == [0x039]); // TAG_BODY_LENGHT
...
pub fn read_standard_trailer(br: &mut dyn BufRead) -> std::io::Result<StandardTrailer> {
    let buf = br.fill_buf()?;
    if buf.len() == 0 {
        ()
    };

    let token = get_field(buf);
...
pub fn get_field_as_string(buf: &[u8]) -> &str {
    let f = get_field(buf);
